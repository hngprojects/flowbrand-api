...
**Never call `res.json()` directly** unless the endpoint performs a `res.redirect()`. Never include `success: true` in what you return — the interceptor adds it, and if you include it manually it lead
...
`business_name` is acceptable in event payloads (e.g. `FunnelGeneratedEvent`) because it serves a legitimate operational purpose — activity logs need it for display. Be aware that sole proprietors som
