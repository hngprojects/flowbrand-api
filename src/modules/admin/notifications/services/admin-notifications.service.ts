...
export class AdminNotificationsService {
  ...
  async markRead(adminId: string, notificationId: string): Promise<AdminNotificationFeedItem> {
    ...
    if (!notification.is_read) {
      ...
      const updated = await this.notificationAction.findOwnedById(notificationId, adminId);
      if (!updated) {
        throw new NotFoundException(SYS_MSG.ADMIN_NOTIFICATION_NOT_FOUND);
      }
  ...
  /** FR-7: toggle is_starred and return the updated notification; 404 when not owned. */
  async toggleStar(adminId: string, notificationId: string): Promise<AdminNotificationFeedItem> {
    ...
    const nextStarred = !notification.is_starred;
    await this.notificationAction.setStarred(notificationId, adminId, nextStarred);

    return this.mapToFeedItem({ ...notification, is_starred: nextStarred });
  }
  ...
  private async findAdminIds(): Promise<string[]> {
    const rows: { user_id: string }[] = await this.userRoleRepository
      .createQueryBuilder('user_role')
      .innerJoin('user_role.user', 'user', 'user.deleted_at IS NULL')
      .select('DISTINCT user_role.user_id', 'user_id')
      .where('user_role.role IN (:...roles)', { roles: ADMIN_ROLES })
      .getRawMany();
