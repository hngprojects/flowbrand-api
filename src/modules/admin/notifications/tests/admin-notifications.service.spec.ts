...
const mockUserRoleRepository = {
  createQueryBuilder: jest.fn(() => mockUserRoleQueryBuilder),
};
...
describe('AdminNotificationsService', () => {
  ...
  describe('markRead', () => {
    it('AC-05: marks an owned unread notification as read', async () => {
      mockNotificationAction.findOwnedById
        .mockResolvedValueOnce(notification())
        .mockResolvedValueOnce(notification({ is_read: true, read_at: new Date() }));
  ...
  describe('toggleStar', () => {
    it('AC-09: flips is_starred from false to true', async () => {
      mockNotificationAction.findOwnedById.mockResolvedValue(notification({ is_starred: false }));
      mockNotificationAction.setStarred.mockResolvedValue(1);

      const result = await service.toggleStar(ADMIN_ID, NOTIFICATION_ID);

      expect(mockNotificationAction.setStarred).toHaveBeenCalledWith(NOTIFICATION_ID, ADMIN_ID, true);
      expect(result.is_starred).toBe(true);
    });

    it('AC-09: flips is_starred back from true to false', async () => {
      mockNotificationAction.findOwnedById.mockResolvedValue(notification({ is_starred: true }));
      mockNotificationAction.setStarred.mockResolvedValue(1);

      const result = await service.toggleStar(ADMIN_ID, NOTIFICATION_ID);

      expect(mockNotificationAction.setStarred).toHaveBeenCalledWith(NOTIFICATION_ID, ADMIN_ID, false);
  ...
  describe('notifyAllAdmins', () => {
    it('FR-9: fans the notification out to every admin and super admin', async () => {
      mockUserRoleQueryBuilder.getRawMany.mockResolvedValue([{ user_id: ADMIN_ID }, { user_id: OTHER_ADMIN_ID }]);
    ...
    it('creates nothing when no admins exist', async () => {
      mockUserRoleQueryBuilder.getRawMany.mockResolvedValue([]);
