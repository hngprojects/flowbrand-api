...
export class GoogleStrategy extends PassportStrategy(Strategy, 'google') {
  ...
  validate(accessToken: string, refreshToken: string, profile: PassportGoogleProfile, done: VerifyCallback): void {
    if (!profile || !profile.emails || !profile.emails.length) {
      return done(new UnauthorizedException(SYS_MSG.GOOGLE_ACCOUNT_NO_EMAIL), false);
    }

    const verifiedEmail = profile.emails.find((e) => e.value && e.verified === true)?.value;
    if (!verifiedEmail) {
      return done(new UnauthorizedException(SYS_MSG.GOOGLE_ACCOUNT_NO_EMAIL), false);
    }
